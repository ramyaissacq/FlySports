...
class KickOffViewController: BaseViewController {
    ...
    var headerLabel = "Fly Sports".localized
    ...
    func setupLeftView(){
        ...
        let leftImage = UIImageView(frame: CGRect(x: 0, y: 0, width: 143, height: 24))
        if Utility.getCurrentLang() == "cn"{
            leftImage.image = UIImage(named: "flyCn")
        }
        else{
        leftImage.image = UIImage(named: "fly")
        }
        if AppPreferences.getMapObject() != nil{
            ...
            self.navigationItem.leftBarButtonItems = [UIBarButtonItem(customView: btn),UIBarButtonItem(customView: leftImage)]
        }
        else{
            self.navigationItem.leftBarButtonItem = UIBarButtonItem(customView: leftImage)
        }
